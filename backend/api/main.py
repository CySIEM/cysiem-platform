...
async def chat_copilot(request: Request, current_user: models.User = Depends(auth.get_current_user)):
    ...
    ans = f"Hello {current_user.username}. I am the CySIEM Intelligence Copilot. How can I assist you today?"
    if "summar" in query:
        ans = "Pipeline processed **1.2M events**. AI Engine flagged **847 alerts**. 2 critical incidents are active (Ransomware, C2 Beacon)."
    elif "remedi" in query or "fix" in query:
        ans = "**Immediate Remediation:**\n1. Execute POST /firewall/block → IP: 45.142.212.100\n2. Isolate endpoint 10.0.2.99 using EDR API."
